...
REQUIRED_COLUMNS = list(RENAME_MAPPING.keys()) + ["Last Update Date", "Last Response Date", "Bank Name"]
OUTPUT_COLUMNS = [
    "Terminal ID",
    "Merchant Name",
    "Standard Merchant",
...
def upload_file():
    ...
    if file and bank_name and ticket_type:
        ...
        try:
            ...
            required_cols_with_bank = list(RENAME_MAPPING.keys()) + ["Last Update Date", "Last Response Date", "Bank Name"]
            missing_cols = [col for col in required_cols_with_bank if col not in df.columns]
            if missing_cols:
            ...
            # Drop the Bank Name column from the final output before saving
            final_df = selected_df.drop(columns=["Last Update Date", "Last Response Date", "Bank Name"])
