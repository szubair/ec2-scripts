...
# Only check for columns that exist in the original Excel file
REQUIRED_COLUMNS = ["TerminalId", "Retailer Name", "TerminalClass", "City", "Within/Outside City", "Ticket Type", "Last Update Date", "Last Response Date", "Bank Name"]

# All columns to be displayed in the final output
OUTPUT_COLUMNS = [
    "Terminal ID",
    "Merchant Name",
    "Standard Merchant",
...
def upload_file():
    ...
    if file and bank_name and ticket_type:
        ...
        try:
            ...
            filtered_df = df[
                (df['Bank Name'].str.contains(bank_name, case=False, na=False)) &
                (df['Ticket Type'].str.contains(ticket_type, case=False, na=False))
            ]
            ...
            selected_df = filtered_df[REQUIRED_COLUMNS].copy()
            ...
            # --- Start of SLA Logic ---
            selected_df['Fulfilment_Time_Hours'] = fulfilment_time_delta.dt.total_seconds() / 3600
            selected_df['SLA_Target_Hours'] = np.where(selected_df['Within/Outside City'].str.lower() == 'within city', 24, 48)
            selected_df['SLA_Status'] = np.where(selected_df['Fulfilment_Time_Hours'] <= selected_df['SLA_Target_Hours'], 'Within SLA', 'Failed SLA')
            selected_df['Above_Target'] = np.where(selected_df['Fulfilment_Time_Hours'] < selected_df['SLA_Target_Hours'], 'Above Target', 'No Failed')
            # --- End of SLA Logic ---
            
            # Drop the columns that are not needed in the final output
            final_df = selected_df.drop(columns=["Last Update Date", "Last Response Date", "Bank Name", "Fulfilment_Time_Hours", "SLA_Target_Hours"])
            
            # Rename columns after all calculations are complete
            final_df = final_df.rename(columns=RENAME_MAPPING)

            # Now, select the final output columns from the DataFrame with the new names
...
def display_results():
    ...
    table_html = paginated_df.to_html(index=False, classes="table-striped", border=0)
